...
use sha2::Sha256;
use url::Url;
use super::errors::BinanceContentError;
...
impl Transport {
    ///
    /// Create New Client Object based
    ...
    fn get_base_url(&self, sandbox: bool) -> String {
        if sandbox {
            "https://testnet.binance.vision/api/v3".into()
        } else {
            "https://api.binance.vision/api/v3".into()
        }
    }

    fn default_headers(api_key: Option<&str>) -> HeaderMap<HeaderValue> {
        let mut headers = HeaderMap::new();
        headers.insert(
            USER_AGENT,
            HeaderValue::from_str("binance-rs/0.1.0").unwrap(),
        );
        headers.insert(
            CONTENT_TYPE,
